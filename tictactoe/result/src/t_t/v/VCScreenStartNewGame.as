...
package t_t.v {
	 ...
	 * @version 0.0.0
	 */
	public class VCScreenStartNewGame extends AVC {
		
		//{ ===== CONSTRUCTOR
		
		function VCScreenStartNewGame (w:uint, h:uint) {
			this.w = w;
			this.h = h;
			prepareContainer();
		...
		private function addSomeInterface():void {
			...
			actionsList = new JPanel(new BoxLayout(BoxLayout.Y_AXIS, 20));
		...
		override public function runSelfTest():void {
			addActionButton(0, APP.lText().get_TEXT(Text.ID_TEXT_GAME_START));
			addActionButton(1, APP.lText().get_TEXT(Text.ID_TEXT_EXIT));
			super.runSelfTest();
		}
		
		private function el_actionButtons(targetID:int):void {
			dispatchEvent(ID_E_B_ACTION, [ID_E_B_START, ID_E_B_EXIT][targetID]);
		}		
		...
		//{ ======= events id 
		public static const ID_E_B_EXIT:String = '>ID_E_B_EXIT';
		public static const ID_E_B_START:String = '>ID_E_B_START';
		...
		*/
		...
		public static const NAME:String = 'VCScreenStartNewGame';
